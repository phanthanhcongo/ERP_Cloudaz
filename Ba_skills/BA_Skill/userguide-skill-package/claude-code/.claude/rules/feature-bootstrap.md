...
3. **Phỏng vấn ĐÚNG PHẠM VI skill đó cần** (KHÔNG hỏi lan man kiểu `/brainstorm` toàn diện — mỗi skill chỉ hỏi cái mình cần để làm output của mình):
   ...
   - `/reverse-doc`: **KHÔNG phỏng vấn, KHÔNG hỏi lại user** (nguồn = proof) — đọc NGUỒN (docx/pdf/ảnh...) rồi cluster thành nhiều feature, derive slug mỗi feature từ nội dung nguồn. Có thể tạo NHIỀU feature 1 lần. Chỗ nguồn thiếu/mơ hồ/mâu thuẫn → ghi OQ/Gap vào `docs/_reverse/{feature}/reverse-gaps.md`, KHÔNG dừng hỏi. Chỉ hỏi DUY NHẤT "nguồn ở đâu" khi no-arg. Xem `reverse-doc/SKILL.md`.
