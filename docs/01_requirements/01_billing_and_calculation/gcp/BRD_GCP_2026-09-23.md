...
1. Mở link billing riêng trên Google Cloud Console:
   - Chọn **Billing** → **Cost Table** (Bảng chi phí).
   - Chọn đúng **Billing Account của GCP Reseller**.
   - Chọn đúng **kỳ cước (tháng)** tương ứng với hóa đơn.

2. Xuất **2 bảng dữ liệu riêng biệt** với cấu hình Group By khác nhau:
   - **Bảng 1** (Project Level): **Group By: Project ID / Project Number** (~621 dòng)
     * Bao gồm tất cả project con của khách hàng trong kỳ
   - **Bảng 2** (Billing Account Level): **Group By: Sub-Account / Billing ID** (~94 dòng)
     * Tổng hợp theo cấp tài khoản thanh toán/Sub-account
   
   > **💡 Chi tiết:** Kết quả 2 bảng phải khớp 100% (tổng Project = tổng Billing Account)
3. **Cấu hình tích chọn (Checkbox)**: Bỏ tích **duy nhất `Reseller Margin`** (lưu ý: `Reseller Margin` và `Negotiated Savings` là 2 checkbox khác nhau); bắt buộc **GIỮ TÍCH `Negotiated Savings`** và **GIỮ TÍCH `Credit`**).
4. **Tiền xử lý file Excel trước Upload**: 
   - Xóa các dòng tiêu đề thừa ở trên cùng file Excel.
   - Xóa cột `% so với tháng trước` (cột hiển thị tỷ lệ tăng/giảm chi phí ở ngoài cùng bên phải).
5. Rà soát thủ công khoản **Promotion Credit**: nếu Credit thuộc về CloudAZ (Google tài trợ) thì xuất Excel xong phải trừ ở cột Credit và cộng bù vào thu/chi công ty; nếu thuộc Khách hàng thì giữ nguyên.
6. Chi phí **Gemini API** (Marketplace — không được chiết khấu 0% Discount): bị Google gộp chung vào tổng chi phí dịch vụ GCP Reseller (không nằm riêng ở cột nào). Kế toán phải mở trang Console/Project của từng khách hàng có dùng Gemini API (~40-50% số lượng khách) để lấy số tiền thực tế và bóc tách tay trước khi tính chiết khấu GCP.
7. Upload 2 file/sheet dữ liệu thô này lên hệ thống CRM.
8. Trên CRM: Gen **Bảng đối soát chi phí** -> Tải Excel về sửa thủ công (tách Gemini API & Credit cty nếu chưa tự động hóa) -> Gửi mail khách. Khách chốt -> Gen **Đề nghị thanh toán (DNTT)** -> Sửa thủ công số tiền -> Xuất PDF gửi khách.
