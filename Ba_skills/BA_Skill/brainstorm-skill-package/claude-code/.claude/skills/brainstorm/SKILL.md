...
11. **Set env trước Write** (hook ghi activity.log): `CLAUDE_SKILL_NAME=/brainstorm` + `CLAUDE_CHANGELOG_AUTHOR={@author}` + `CLAUDE_CHANGELOG_NOTE` (≤80 ký tự, vd `initial brainstorm scaffold cho {title}, deep mode, {N} OQs`). Hook ghép cả dòng — skill KHÔNG tự viết `{date} | ... | {note}`.
...
13b. **Phase E2 — Mark "đã chi tiết hóa" ngược lên PRD sản phẩm** — sau khi brainstorm write xong:
    ...
    - **Tìm thấy row** + cột Chi tiết hóa chưa phải `✅` → đếm `docs/{feature}/brainstorms/*.md` (N file) → **propose L2 diff** đổi cột Chi tiết hóa của row đó sang `✅ đã chi tiết (N brainstorm)`. User Y → Edit brief + set env `CLAUDE_CHANGELOG_NOTE=mark {feature} đã chi tiết hóa ({N} brainstorm)` (+ SKILL_NAME/AUTHOR) trước Edit; hook ghi activity.log.
